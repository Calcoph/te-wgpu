...
impl GlobalPlay for wgc::global::Global {
    fn encode_commands(
        ...
        let cmd_buf = self.command_encoder_finish(
            encoder,
    ...
    fn process(
        ...
        match action {
            ...
            Action::CreateBuffer(id, desc) => {
                self.device_create_buffer(device, &desc, Some(id))
                    .unwrap();
            }
            ...
            Action::CreateTexture(id, desc) => {
                self.device_create_texture(device, &desc, Some(id))
                    .unwrap();
            }
            ...
            Action::CreateTextureView {
                ...
                self.texture_create_view(parent_id, &desc, Some(id))
                    .unwrap();
            }
            ...
            Action::CreateExternalTexture { id, desc, planes } => {
                self.device_create_external_texture(device, &desc, &planes, Some(id)).unwrap();
            }
            ...
            Action::CreateSampler(id, desc) => {
                self.device_create_sampler(device, &desc, Some(id))
                    .unwrap();
            }
            ...
            Action::CreateBindGroupLayout(id, desc) => {
                self.device_create_bind_group_layout(device, &desc, Some(id))
                    .unwrap();
            }
            ...
            Action::CreatePipelineLayout(id, desc) => {
                self.device_create_pipeline_layout(device, &desc, Some(id))
                    .unwrap();
            }
            ...
            Action::CreateBindGroup(id, desc) => {
                self.device_create_bind_group(device, &desc, Some(id))
                    .unwrap();
            }
            ...
            Action::CreateShaderModule { id, desc, data } => {
                ...
                let res = self.device_create_shader_module(device, &desc, source, Some(id));
                if let Err(e) = res {
            ...
            Action::CreateComputePipeline { id, desc } => {
                self.device_create_compute_pipeline(device, &desc, Some(id)).unwrap();
            }
            Action::DestroyComputePipeline(id) => {
                self.compute_pipeline_drop(id);
            }
            Action::CreateRenderPipeline { id, desc } => {
                self.device_create_render_pipeline(device, &desc, Some(id)).unwrap();
            }
            Action::CreateMeshPipeline { id, desc } => {
                self.device_create_mesh_pipeline(device, &desc, Some(id)).unwrap();
            }
            ...
            Action::CreateRenderBundle { id, desc, base } => {
                ...
                self.render_bundle_encoder_finish(
                    bundle,
            ...
            Action::CreateQuerySet { id, desc } => {
                self.device_create_query_set(device, &desc, Some(id))
                    .unwrap();
            }
            ...
            Action::Submit(_index, commands) => {
                let encoder = self.device_create_command_encoder(
                    device,
            ...
            Action::CreateBlas { id, desc, sizes } => {
                self.device_create_blas(device, &desc, sizes, Some(id)).unwrap();
            }
            Action::DestroyBlas(id) => {
                self.blas_drop(id);
            }
            Action::CreateTlas { id, desc } => {
                self.device_create_tlas(device, &desc, Some(id)).unwrap();
            }
