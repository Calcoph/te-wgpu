...
fn create_external_texture() {
    ...
    let r_view = r_texture.create_view(&TextureViewDescriptor::default()).unwrap();
    let rg_texture = device.create_texture(&TextureDescriptor {
        format: TextureFormat::Rg8Unorm,
        ..texture_descriptor
    }).unwrap();
    let rg_view = rg_texture.create_view(&TextureViewDescriptor::default()).unwrap();
    let rgba_texture = device.create_texture(&TextureDescriptor {
        format: TextureFormat::Rgba8Unorm,
        ..texture_descriptor
    }).unwrap();
    let rgba_view = rgba_texture.create_view(&TextureViewDescriptor::default()).unwrap();
    ...
    let uint_view = uint_texture.create_view(&TextureViewDescriptor::default()).unwrap();
    let _ = fail(
        || {
            device.create_external_texture(
                &ExternalTextureDescriptor {
    ...
    let d3_view = d3_texture.create_view(&TextureViewDescriptor::default()).unwrap();
    let _ = fail(
        || {
            device.create_external_texture(
                &ExternalTextureDescriptor {
    ...
    let multisampled_view = multisampled_texture.create_view(&TextureViewDescriptor::default()).unwrap();
    let _ = fail(
        || {
            device.create_external_texture(
                &ExternalTextureDescriptor {
    ...
    let non_binding_view = non_binding_texture.create_view(&TextureViewDescriptor::default()).unwrap();
    let _ = fail(
        || {
            device.create_external_texture(
                &ExternalTextureDescriptor {
...
fn external_texture_binding() {
    ...
    valid(|| {
        ...
        let external_texture =
            device.create_external_texture(&external_texture_descriptor, &[&view]).unwrap();
...
fn destroyed_external_texture_plane() {
    ...
    let target_view = target_texture.create_view(&TextureViewDescriptor::default()).unwrap();
    ...
    let plane_view = plane_texture.create_view(&TextureViewDescriptor::default()).unwrap();
...
@fragment fn frag_main() -> @location(0) vec4<f32> { return textureLoad(tex, vec2(0)); }",
    ...
    let mut encoder = device.create_command_encoder(&CommandEncoderDescriptor { label: None }).unwrap();
    ...
    pass.draw(0..0, 0..0).unwrap();
    drop(pass);

    plane_texture.destroy();

    fail(
        || queue.submit([encoder.finish().unwrap()]).map_err(|(_, e)| e),
