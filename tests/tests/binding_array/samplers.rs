...
async fn binding_array_samplers(ctx: TestingContext, partially_bound: bool) {
    ...
    let input_view = texture.create_view(&TextureViewDescriptor::default()).unwrap();
    ...
    encoder.copy_buffer_to_buffer(&output_buffer, 0, &readback_buffer, 0, 4 * 2).unwrap();

    ctx.queue.submit(Some(encoder.finish().unwrap())).unwrap();

    readback_buffer.slice(..).map_async(MapMode::Read, |_| {}).unwrap();
