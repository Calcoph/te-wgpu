...
async fn set_array_stride_to_0(ctx: TestingContext) {
    ...
    let first_pipeline = ctx.device.create_render_pipeline(&first_pipeline_desc).unwrap();
...

    {
        ...
        rpass.set_pipeline(&first_pipeline).unwrap(); // This call caused the D3D12 backend to set the stride for the 2nd vertex buffer to 16.
        rpass.set_pipeline(&pipeline).unwrap(); // This call doesn't set the stride for the 2nd vertex buffer to 0.
        rpass.set_vertex_buffer(0, position_buffer.slice(..)).unwrap();
        rpass.set_vertex_buffer(1, color_buffer.slice(..)).unwrap();
        rpass.draw(0..6, 0..1).unwrap(); // Causing this draw to be skipped since it would read OOB of the 2nd vertex buffer.
    }
