...
fn create_depth(
    ...
    let depth_view = depth_texture.create_view(&Default::default()).unwrap();
...
fn mesh_pipeline_build(ctx: &TestingContext, info: MeshPipelineTestInfo) {
    ...
    if info.draw {
            ...
            device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None }).unwrap();
        {
            ...
            pass.draw_mesh_tasks(1, 1, 1).unwrap();
        }
...
fn mesh_draw(ctx: &TestingContext, draw_type: DrawType) {
        ...
        device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None }).unwrap();
    {
        ...
        pass.set_pipeline(&pipeline).unwrap();
        match draw_type {
            DrawType::Standard => pass.draw_mesh_tasks(1, 1, 1).unwrap(),
            DrawType::Indirect => pass.draw_mesh_tasks_indirect(buffer.as_ref().unwrap(), 0).unwrap(),
            DrawType::MultiIndirect => {
                pass.multi_draw_mesh_tasks_indirect(buffer.as_ref().unwrap(), 0, 1).unwrap()
            }
        ...
        pass.draw_mesh_tasks_indirect(buffer.as_ref().unwrap(), 0).unwrap();
    }
    ctx.queue.submit(Some(encoder.finish().unwrap())).unwrap();
    ctx.device
        .poll(wgpu::PollType::wait_indefinitely())
