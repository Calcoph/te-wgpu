...
#[derive(Debug)]
pub struct TextureBlitterError;
...
impl<'a> TextureBlitterBuilder<'a> {
    ...
    pub fn build(self) -> Result<TextureBlitter, TextureBlitterError> {
        ...
        let shader = self.device.create_shader_module(include_wgsl!("blit.wgsl")).map_err(|_| TextureBlitterError)?;
        let pipeline = self
            .device
...
impl TextureBlitter {
    ...
    pub fn new(device: &Device, format: TextureFormat) -> Result<Self, TextureBlitterError> {
