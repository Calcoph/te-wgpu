...
impl Surface<'_> {
    ...
    pub fn configure(&self, device: &Device, config: &SurfaceConfiguration) -> Result<(), present::ConfigureSurfaceError>{
    ...
    pub fn get_current_texture(&self) -> Result<Result<SurfaceTexture, SurfaceError>, present::SurfaceError> {
        ...
        let suboptimal = match status {
            ...
            SurfaceStatus::Timeout => return Ok(Err(SurfaceError::Timeout)),
            SurfaceStatus::Outdated => return Ok(Err(SurfaceError::Outdated)),
            SurfaceStatus::Lost => return Ok(Err(SurfaceError::Lost)),
            SurfaceStatus::Unknown => return Ok(Err(SurfaceError::Other)),
        };
