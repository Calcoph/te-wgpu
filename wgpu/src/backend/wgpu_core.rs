...
impl ContextWgpuCore {
    ...
    ) -> Result<(wgc::id::BufferId, Buffer), CreateBufferError> {
        let id = unsafe {
...
impl crate::Context for ContextWgpuCore {
    ...
    fn adapter_request_device(
        ...
        let (device_id, queue_id) = match wgc::gfx_select!(*adapter => self.0.adapter_request_device(
            *adapter,
            &desc.map_label(|l| l.map(Borrowed)),
            None,
            None,
    ...
    fn adapter_is_surface_supported(
        ...
        match wgc::gfx_select!(adapter => self.0.adapter_is_surface_supported(*adapter, *surface)) {
            Ok(result) => result,
            Err(err) => panic!("{err}Adapter::is_surface_supported"),
        }
    }
    ...
    fn adapter_features(
        ...
        match wgc::gfx_select!(*adapter => self.0.adapter_features(*adapter)) {
            ...
            Err(err) => panic!("{err}Adapter::features"),
        }
    }
    ...
    fn adapter_limits(
        ...
        match wgc::gfx_select!(*adapter => self.0.adapter_limits(*adapter)) {
            Ok(limits) => limits,
            Err(err) => panic!("{err}Adapter::limits"),
        }
    }
    ...
    fn adapter_downlevel_capabilities(
        ...
        match wgc::gfx_select!(*adapter => self.0.adapter_downlevel_capabilities(*adapter)) {
            ...
            Err(err) => panic!("{err}Adapter::downlevel_properties"),
        }
    }
    ...
    fn adapter_get_info(
        ...
        match wgc::gfx_select!(*adapter => self.0.adapter_get_info(*adapter)) {
            Ok(info) => info,
            Err(err) => panic!("{err}Adapter::get_info"),
        }
    }
    ...
    fn adapter_get_texture_format_features(
        ...
        {
            Ok(info) => info,
            Err(err) => panic!("{err}Adapter::get_texture_format_features"),
        }
    }
    ...
    fn adapter_get_presentation_timestamp(
        ...
        match wgc::gfx_select!(*adapter => self.0.adapter_get_presentation_timestamp(*adapter)) {
            ...
            Err(err) => panic!("{err}Adapter::correlate_presentation_timestamp"),
        }
    }
    ...
    fn device_features(
        ...
        match wgc::gfx_select!(device => self.0.device_features(*device)) {
            Ok(features) => features,
            Err(err) => panic!("{err}Device::features"),
        }
    }
    ...
    fn device_limits(&self, device: &Self::DeviceId, _device_data: &Self::DeviceData) -> Limits {
        match wgc::gfx_select!(device => self.0.device_limits(*device)) {
            Ok(limits) => limits,
            Err(err) => panic!("{err}Device::limits"),
        }
    }
    ...
    fn device_downlevel_properties(
        ...
        match wgc::gfx_select!(device => self.0.device_downlevel_properties(*device)) {
            Ok(limits) => limits,
            Err(err) => panic!("{err}Device::downlevel_properties"),
        }
    }
    ...
    fn device_create_shader_module(
    ...
    ) -> Result<(Self::ShaderModuleId, Self::ShaderModuleData), CreateShaderModuleError> {
        ...
        Ok((id, ShaderModule { compilation_info }))
    }
    ...
    fn device_create_bind_group(
    ...
    ) -> Result<(Self::BindGroupId, Self::BindGroupData), CreateBindGroupError> {
        ...
        let id = wgc::gfx_select!(device => self.0.device_create_bind_group(
            *device,
            &descriptor,
            None
        ))?;
    ...
    fn device_create_pipeline_layout(
    ...
    ) -> Result<(Self::PipelineLayoutId, Self::PipelineLayoutData), CreatePipelineLayoutError> {
        ...
        let id = wgc::gfx_select!(device => self.0.device_create_pipeline_layout(
            *device,
            &descriptor,
            None
        ))?;
    ...
    fn device_create_render_pipeline(
    ...
    ) -> Result<(Self::RenderPipelineId, Self::RenderPipelineData), CreateRenderPipelineError> {
        ...
        let id = match wgc::gfx_select!(device => self.0.device_create_render_pipeline(
            *device,
            &descriptor,
            None,
            None,
    ...
    fn device_create_compute_pipeline(
        ...
        let id = match wgc::gfx_select!(device => self.0.device_create_compute_pipeline(
            *device,
            &descriptor,
            None,
            None,
    ...
    ) -> Result<(Self::PipelineCacheId, Self::PipelineCacheData), pipeline::CreatePipelineCacheError> {
        use wgc::pipeline as pipe;
    ...
    ) -> Result<(Self::BufferId, Self::BufferData), CreateBufferError> {
        let id = wgc::gfx_select!(device => self.0.device_create_buffer(
            *device,
            &desc.map_label(|l| l.map(Borrowed)),
            None
        ))?;
    ...
    fn device_create_texture(
        ...
        _device_data: &Self::DeviceData,
        desc: &TextureDescriptor<'_>,
    ) -> Result<(Self::TextureId, Self::TextureData), CreateTextureError> {
        let wgt_desc = desc.map_label_and_view_formats(|l| l.map(Borrowed), |v| v.to_vec());
        let id = wgc::gfx_select!(device => self.0.device_create_texture(
            *device,
            &wgt_desc,
            None
        ))?;
    ...
    fn device_create_sampler(
    ...
    ) -> Result<(Self::SamplerId, Self::SamplerData), CreateSamplerError> {
        ...
        let id = wgc::gfx_select!(device => self.0.device_create_sampler(
            *device,
            &descriptor,
            None
        ))?;
    ...
    ) -> Result<(Self::QuerySetId, Self::QuerySetData), CreateQuerySetError> {
        let id = wgc::gfx_select!(device => self.0.device_create_query_set(
            *device,
            &desc.map_label(|l| l.map(Borrowed)),
            None
        ))?;
    ...
    ) -> Result<(Self::CommandEncoderId, Self::CommandEncoderData), DeviceError> {
        let id = wgc::gfx_select!(device => self.0.device_create_command_encoder(
            *device,
            &desc.map_label(|l| l.map(Borrowed)),
            None
        ))?;
    ...
    fn buffer_unmap(&self, buffer: &Self::BufferId, buffer_data: &Self::BufferData) -> Result<(), wgc::resource::BufferAccessError> {
        wgc::gfx_select!(buffer => self.0.buffer_unmap(*buffer))
    }
    ...
    ) -> Result<(Self::TextureViewId, Self::TextureViewData), CreateTextureViewError> {
    ...
    fn command_encoder_copy_buffer_to_buffer(
        ...
        wgc::gfx_select!(encoder => self.0.command_encoder_copy_buffer_to_buffer(
            *encoder,
            *source,
            source_offset,
            *destination,
    ...
    fn command_encoder_copy_buffer_to_texture(
        ...
        wgc::gfx_select!(encoder => self.0.command_encoder_copy_buffer_to_texture(
            *encoder,
    ...
    fn command_encoder_copy_texture_to_buffer(
        ...
        wgc::gfx_select!(encoder => self.0.command_encoder_copy_texture_to_buffer(
            *encoder,
    ...
    fn command_encoder_copy_texture_to_texture(
        ...
        wgc::gfx_select!(encoder => self.0.command_encoder_copy_texture_to_texture(
            *encoder,
            &map_texture_copy_view(source),
            &map_texture_copy_view(destination),
            &copy_size
    ...
    fn command_encoder_begin_compute_pass(
        ...
        let pass = gfx_select!(encoder => self.0.command_encoder_create_compute_pass_dyn(*encoder, &wgc::command::ComputePassDescriptor {
            label: desc.label.map(Borrowed),
    ...
    fn command_encoder_begin_render_pass(
        ...
        if desc.color_attachments.len() > wgc::MAX_COLOR_ATTACHMENTS {
            let err = wgc::command::ColorAttachmentError::TooMany {
                given: desc.color_attachments.len(),
                limit: wgc::MAX_COLOR_ATTACHMENTS,
            };
            panic!("{err}CommandEncoder::begin_render_pass");
        }
        let colors = desc
            .color_attachments
            .iter()
        ...
        let pass = gfx_select!(encoder => self.0.command_encoder_create_render_pass_dyn(*encoder, &wgc::command::RenderPassDescriptor {
            label: desc.label.map(Borrowed),
            timestamp_writes: timestamp_writes.as_ref(),
    ...
    ) -> Result<(Self::CommandBufferId, Self::CommandBufferData), CommandEncoderError> {
        let descriptor = wgt::CommandBufferDescriptor::default();
        encoder_data.open = false; // prevent the drop
        let id = wgc::gfx_select!(encoder => self.0.command_encoder_finish(encoder, &descriptor))?;
    ...
    fn command_encoder_clear_texture(
        ...
        wgc::gfx_select!(encoder => self.0.command_encoder_clear_texture(
            *encoder,
            texture.id.into(),
            subresource_range
        ))
    ...
    fn command_encoder_clear_buffer(
        ...
        wgc::gfx_select!(encoder => self.0.command_encoder_clear_buffer(
            *encoder,
            buffer.id.into(),
            offset, size
        ))
    ...
    fn command_encoder_write_timestamp(
        ...
        wgc::gfx_select!(encoder => self.0.command_encoder_write_timestamp(
            *encoder,
            *query_set,
            query_index
        ))
    ...
    fn command_encoder_resolve_query_set(
        ...
        wgc::gfx_select!(encoder => self.0.command_encoder_resolve_query_set(
            *encoder,
            *query_set,
            first_query,
            query_count,
    ...
    fn render_bundle_encoder_finish(
        ...
        let id = wgc::gfx_select!(encoder_data.parent() => self.0.render_bundle_encoder_finish(
            encoder_data,
            &desc.map_label(|l| l.map(Borrowed)),
            None
        ))
    ...
    fn queue_write_texture(
        ...
        wgc::gfx_select!(*queue => self.0.queue_write_texture(
            *queue,
    ...
    fn queue_submit<I: Iterator<Item = (Self::CommandBufferId, Self::CommandBufferData)>>(
        ...
        {
            Ok(index) => index,
            Err(err) => panic!("{err}Queue::submit"),
        };
    ...
    fn queue_get_timestamp_period(
        ...
        match res {
            Ok(v) => v,
            Err(cause) => {
                panic!("{cause}Queue::get_timestamp_period");
            }
        }
    }
    ...
    fn queue_on_submitted_work_done(
        ...
        if let Err(cause) = res {
            panic!("{cause}Queue::on_submitted_work_done");
        }
    }
    ...
    fn compute_pass_set_pipeline(
        ...
        pass_data.pass.set_pipeline(&self.0, *pipeline)
    }
    ...
    fn compute_pass_set_push_constants(
        ...
        pass_data.pass.set_push_constants(&self.0, offset, data)
    }
    ...
    fn render_pass_multi_draw_indexed_indirect(
        ...
        pass_data.pass.multi_draw_indexed_indirect(
            &self.0,
            *indirect_buffer,
            indirect_offset,
            count,
    ...
    fn render_pass_multi_draw_indirect_count(
        ...
        pass_data.pass.multi_draw_indirect_count(
            &self.0,
            *indirect_buffer,
            indirect_offset,
            *count_buffer,
    ...
    fn render_pass_multi_draw_indexed_indirect_count(
        ...
        pass_data.pass.multi_draw_indexed_indirect_count(
            &self.0,
            *indirect_buffer,
            indirect_offset,
            *count_buffer,
    ...
    fn render_pass_set_blend_constant(
        ...
        pass_data.pass.set_blend_constant(&self.0, color)
    }
    ...
    fn render_pass_set_stencil_reference(
        ...
        pass_data.pass.set_stencil_reference(&self.0, reference)
    }
    ...
    fn render_pass_begin_occlusion_query(
        ...
        pass_data.pass.begin_occlusion_query(&self.0, query_index)
    }
    ...
    fn render_pass_end_occlusion_query(
        ...
        pass_data.pass.end_occlusion_query(&self.0)
    }
