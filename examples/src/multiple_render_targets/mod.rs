...
impl TargetRenderer {
    ...
    fn draw(
        ...
        rpass.set_viewport(0.0, 0.0, half_w, height, 0.0, 1.0).unwrap();
        rpass.draw(0..3, 0..1).unwrap();

        rpass.set_viewport(half_w, 0.0, half_w, height, 0.0, 1.0).unwrap();
        rpass.set_bind_group(0, &self.bindgroup_right, &[]).unwrap();
        rpass.draw(0..3, 0..1).unwrap();
    }
